...
export default class MonitorScreen extends EventEmitter {
    ...
    createIframe() {
        ...
        // Ekran Kaynağı (Dış sunucu yerine yerel arayüz veya özel portfolyo URL'si)
        const urlParams = new URLSearchParams(window.location.search);
        if (urlParams.has('screenUrl')) {
            iframe.src = urlParams.get('screenUrl')!;
        } else if (urlParams.has('dev')) {
            iframe.src = 'http://localhost:3000/';
        } else {
            // Proje derlendiğinde static/screen/ altından çalışacak yerel sistem
        ...
        iframe.title = 'Personal Desktop OS';
     ...
     */
    createEnclosingPlane(plane: { size: THREE.Vector2; position: THREE.Vector3; rotation: THREE.Euler }) {
        const material = new THREE.MeshBasicMaterial({
            side: THREE.DoubleSide,
            color: 0x48493f,
        });
